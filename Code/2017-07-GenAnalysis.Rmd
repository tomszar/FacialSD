...
Creating a new column to plot 1000Genome pops against our samples more easily.
...
for(i in 1:nrow(pca.eigenvec)){
  if(pca.eigenvec$IID[i] %in% pops$IID){
    ...
    pca.eigenvec$FID[i] <- as.character(pops$super_pop[ind])
  }
}
pca.eigenvec$FID <- as.factor(pca.eigenvec$FID)
pca.eigenvec$database <- pca.eigenvec$FID 
pca.eigenvec$database[!(pca.eigenvec$FID %in% pops$super_pop)] <- "ADAPT"
pca.eigenvec$database <- droplevels(pca.eigenvec$database)
```
...
eurhap  <- filter(meanshap, database == "EUR")
...
#plotQ(admix,showindlab=T,
#     ordergrp=T,showlegend=T,
