...
We can use joy plots to compare the distributions of each PC for each sex.

```{r joyplots, fig.height = 10, fig.width = 8, fig.align = "center", message=FALSE}
source("PlotPCs.R")
p1 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex ,"PC1", "PC9")
p2 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC10", "PC19")
p3 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC20", "PC29")
p4 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC30", "PC39")
ggarrange(p1, p2, p3, p4,
          common.legend = T, legend = "bottom")

p1 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC40", "PC49")
p2 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC50", "PC59")
p3 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC60", "PC69")
p4 <- PlotPCs(coeffs[, c(6:ncol(coeffs)) ], coeffs$Sex, "PC70", "PC79")
ggarrange(p1, p2, p3, p4, 
          common.legend = T, legend = "bottom")
```
