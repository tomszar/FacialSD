...
p1 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "Africa", "PC1", "PC2")
p2 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "Africa", "PC3", "PC4")
p3 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "America", "PC1", "PC2")
p4 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "America", "PC3", "PC4")
p5 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "Asia", "PC1", "PC2")
p6 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "Asia", "PC3", "PC4")
p7 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "NEruope", "PC1", "PC2")
p8 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "NEruope", "PC3", "PC4")
p9 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "SEruope", "PC1", "PC2")
p10 <- plotsexes(MergedDat[[2]], MergedDat[[1]]$continent == "SEruope", "PC3", "PC4")
...
It is clear from the scatter plots that Asian faces differ substantially in their sample size. 
...
**Note:** What if using mahalanobis distance instead of euclidean, to take unequal sample sizes into account?

```{r MANOVA}
library(vegan)
pca <- as.matrix(dplyr::filter(MergedDat[[2]], MergedDat[[1]]$continent != "Asia"))
sex <- dplyr::filter(MergedDat[[1]], continent != "Asia" )$Sex
continent <- dplyr::filter(MergedDat[[1]], continent != "Asia" )$continent

fit       <- manova(pca ~ sex * continent)
summary(fit, test="Pillai")
fit2 <- adonis(pca ~ sex * continent, permutations = 999, 
               method = "euclidean")
fit2
```

We can also compute pairwise mahalanobis distance as a measure of effect size.
This is preffered from an Euclidean distance, because the within-group variation is taken into account in the mahalanobis distance.
As shown in the dendogram, Asia is the least similar groups overall.
