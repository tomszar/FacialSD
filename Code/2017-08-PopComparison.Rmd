...
```{r summary plots, message=F}
p1 <- ggplot(MergedDat[[1]], aes(x = interaction(Sex, continent), y = Age)) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x="")
p2 <- ggplot(MergedDat[[1]], aes(x = interaction(Sex, continent), y = Height)) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x="")
p3 <- ggplot(MergedDat[[1]], aes(x = interaction(Sex, continent), y = Weight)) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + labs(x="")
ggarrange(p1, p2, p3)
```
...
fit10 <- manova(as.matrix(MergedDat[[2]][,1:10]) ~ MergedDat[[1]]$Sex * MergedDat[[1]]$continent)
summary(fit10)
...
#ALLOMETRIC SD
fit <- lm(as.matrix(MergedDat[[2]]) ~ MergedDat[[1]]$Sex + MergedDat[[1]]$Weight +
            MergedDat[[1]]$Height * MergedDat[[1]]$continent)
summary(manova(fit))
alloSD <- totalSD
for(cont in 1:length(levels(MergedDat[[1]]$continent)))
{
  pca  <- dplyr::filter(MergedDat[[2]], 
                        MergedDat[[1]]$continent == levels(MergedDat[[1]]$continent)[cont])
...
```{r angles, message=F}
source("Distances.R")
labscont   <- levels(consensus$continent)

totangle   <- getPAngle(totalSD[,2:88])
totangle10 <- getPAngle(totalSD[,2:11])
plot(hclust(as.dist(totangle)), labels = labscont, main = "Cluster Dendrogram Total SD")
plot(hclust(as.dist(totangle10)), labels = labscont, main = "Cluster Dendrogram Total SD, subspace")

alloang   <- getPAngle(alloSD[,2:88])
alloang10 <- getPAngle(alloSD[,2:11])
plot(hclust(as.dist(alloang)), labels = labscont, main = "Cluster Dendrogram Allometric SD")
plot(hclust(as.dist(alloang10)), labels = labscont, main = "Cluster Dendrogram Allometric SD, subspace")

nalloang   <- getPAngle(nonalloSD[,2:88])
nalloang10 <- getPAngle(nonalloSD[,2:11])
plot(hclust(as.dist(nalloang)), labels = labscont)
plot(hclust(as.dist(nalloang10)), labels = labscont)
...
#sum(ma[1,6] < nulls[5,])/1000
#sum(ma[2,3] < nulls[6,])/1000
#sum(ma[2,4] < nulls[7,])/1000
#sum(ma[2,5] < nulls[8,])/1000
#sum(ma[2,6] < nulls[9,])/1000
#sum(ma[3,4] < nulls[10,])/1000
#sum(ma[3,5] < nulls[11,])/1000
#sum(ma[3,6] < nulls[12,])/1000
#sum(ma[4,5] < nulls[13,])/1000
#sum(ma[4,6] < nulls[14,])/1000
#sum(ma[5,6] < nulls[15,])/1000
...
```{r save}
final      <- rbind(totalSD, alloSD, nonalloSD)
final$type <- c(rep("Total", 6), rep("Allo", 6), rep("NonAllo", 6))
setwd(paste(path, "/Results/FSD", sep = ""))
write.csv(final, file="popvectors.csv", quote = F, row.names = F)
```