...
fit
fit <- adonis(coeffs[ , 6:ncol(coeffs)] ~ coeffs$Sex, method="mahalanobis")
fit
```
...
In this section I will decompose the total facial sexual dimorphims (FSD), defined as the difference between the average female and average male location, into its allometric and non-allometric components.
To do that, first we will create consensus faces, taking the average male and female.
...
p1 <- plotmeans(coeffs[,6:ncol(coeffs)], consensus, coeffs$Sex, "PC1", "PC2")
p2 <- plotmeans(coeffs[,6:ncol(coeffs)], consensus, coeffs$Sex, "PC3", "PC4")
p3 <- plotmeans(coeffs[,6:ncol(coeffs)], consensus, coeffs$Sex, "PC5", "PC6")
p4 <- plotmeans(coeffs[,6:ncol(coeffs)], consensus, coeffs$Sex, "PC7", "PC8")
p5 <- plotmeans(coeffs[,6:ncol(coeffs)], consensus, coeffs$Sex, "PC9", "PC10")
p6 <- plotmeans(coeffs[,6:ncol(coeffs)], consensus, coeffs$Sex, "PC11", "PC12")
...
To estimate the total FSD we subtract the male consensus face to the female one
...
source("PlotPCs.R")
p1 <- plotvect(coeffs[,6:ncol(coeffs)], rownames(consensus)[3:5], consensus[3:5,], "PC1", "PC2")
p2 <- plotvect(coeffs[,6:ncol(coeffs)], rownames(consensus)[3:5], consensus[3:5,], "PC3", "PC4")
p3 <- plotvect(coeffs[,6:ncol(coeffs)], rownames(consensus)[3:5], consensus[3:5,], "PC5", "PC6")
p4 <- plotvect(coeffs[,6:ncol(coeffs)], rownames(consensus)[3:5], consensus[3:5,], "PC7", "PC8")
p5 <- plotvect(coeffs[,6:ncol(coeffs)], rownames(consensus)[3:5], consensus[3:5,], "PC9", "PC10")
p6 <- plotvect(coeffs[,6:ncol(coeffs)], rownames(consensus)[3:5], consensus[3:5,], "PC11", "PC12")
