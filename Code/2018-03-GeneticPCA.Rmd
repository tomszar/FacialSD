...
We will run a distance estimation using the first 10 PCs.
From that, we will create a hierarchical clustering and select the different populations as K.

```{r}
eudist <- dist(gen.eigenvec[gen.eigenvec$super_pop == "ADAPT", 3:12])
#cutree(hclust(eudist), k = 5)
plot(hclust(eudist))
```
